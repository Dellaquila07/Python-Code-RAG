...
- This reduces hallucinations by constraining the LLM to repository-specific evidence. 🛡️

### 2. AST-Based Static Code Analysis (Python) 🐍
- Python files are parsed with the built-in `ast` module. 📊
- Functions and classes are extracted as structured blocks with: 🏗️
  - symbol name, 🏷️
  - block type, 📋
  - file path, 📁
  - line boundaries, 📏
...
### 5. Vector Database Indexing with ChromaDB 🗄️
...
### 7. Dependency Container Pattern 🏗️
...
- `app/core/`: configuration and shared dependencies ⚙️
...
- "How does the error handling flow work?" ⚠️
